...
function UserMenu(){
    ...
    if(!user) return (
        <>
            <li className="nav-item me-2"><Link to="/login" className="nav-link">Se connecter</Link></li>
    ...
    return (
        <li className="nav-item dropdown">
            <span className="nav-link dropdown-toggle" role="button">{user.name}</span>
            <div className="dropdown-menu dropdown-menu-end shadow" style={{minWidth:160}}>
                {user.role === 'ADMIN' && <Link to="/admin" className="dropdown-item">Admin</Link>}
                <button className="dropdown-item" onClick={doLogout}>Se déconnecter</button>
            </div>
        </li>
    );
}
                ...
                <div className="container">
                    ...
                    <div className="collapse navbar-collapse show" id="navbarSupportedContent">
                        <ul className="navbar-nav me-auto mb-2 mb-md-0">
                            <li className="nav-item"><Link to="/biscuits" className="nav-link">Biscuits</Link></li>
                            <li className="nav-item"><Link to="/saveurs" className="nav-link">Saveurs</Link></li>
                            <li className="nav-item"><Link to="/commentaires" className="nav-link">Commentaires</Link></li>
                            <li className="nav-item"><Link to="/about" className="nav-link">À propos</Link></li>
                        </ul>
            ...
            <main className="py-4">
                <div className="container" style={{ maxWidth: 1200, padding: '20px' }}>
                    <Routes>
                        ...
                        <Route path="/commentaires" element={<CommentairesPublic/>} />
                        <Route path="/commandes/create" element={<CommandeCreate/>} />
                        {/* Admin routes (protected) */}
                        <Route path="/admin" element={<ProtectedRoute adminOnly><AdminDashboard/></ProtectedRoute>} />
                        <Route path="/admin/biscuits" element={<ProtectedRoute adminOnly><AdminBiscuits/></ProtectedRoute>} />
                        <Route path="/admin/saveurs" element={<ProtectedRoute adminOnly><AdminSaveurs/></ProtectedRoute>} />
                        <Route path="/admin/commandes" element={<ProtectedRoute adminOnly><AdminCommandes/></ProtectedRoute>} />
                    </Routes>
                </div>
            </main>
            </HashRouter>
